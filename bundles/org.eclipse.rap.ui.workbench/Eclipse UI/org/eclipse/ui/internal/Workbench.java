...
public final class Workbench extends EventManager implements IWorkbench,
	...
	private static final String KEY_WORKBENCH_STATE = Workbench.class.getName() + "#XMLMemento";

	// RAP [fappel]: ensure that workbench is properly shutdown in case of session
	// timeout
	...
	private Workbench() {
		...
		serviceLocator = (ServiceLocator) slc.createServiceLocator(null, null, () -> {
			...
			if (display != null && !display.isDisposed()) {
				MessageDialog.openInformation(null, WorkbenchMessages.get().Workbench_NeedsClose_Title,
...
//				E4Workbench e4Workbench = e4app.createE4Workbench(getApplicationContext(), display);
				...
				E4Workbench e4Workbench = e4app.createE4Workbench(
						getApplicationContext(
								E4ApplicationConfig.create("org.eclipse.rap.ui.workbench/LegacyIDE.e4xmi", true)),
						display);
				...
				workbench.serviceLocator.registerService(IWorkbenchLocationService.class, new WorkbenchLocationService(
						IServiceScopes.WORKBENCH_SCOPE, workbench, null, null, null, null, 0));
...
//	}
	...
	public ContributionInfo[] getEarlyActivatedPlugins() {
		...
		IExtensionPoint point = registry
				.getExtensionPoint(PlatformUI.PLUGIN_EXTENSION_NAME_SPACE, IWorkbenchRegistryConstants.PL_STARTUP);
...
//					ContributionInfoMessages.ContributionInfo_EarlyStartupPlugin, null);
			result[i] = new ContributionInfo(pluginIds.get(i),
					ContributionInfoMessages.get().ContributionInfo_EarlyStartupPlugin, null);
		...
		Job job = new Job("Executing the early startup extensions") { //$NON-NLS-1$
			...
			protected IStatus run(IProgressMonitor monitor) {
				...
				RWT.getUISession(display).exec(new Runnable() {
					@Override
					public void run() {
						HashSet<String> disabledPlugins = new HashSet<>(
								Arrays.asList(getDisabledEarlyActivatedPlugins()));
						SubMonitor subMonitor = SubMonitor.convert(monitor,
								WorkbenchMessages.get().Workbench_startingPlugins, extensions.length);
						for (IExtension extension : extensions) {
